...
// *************************************************
var spawn_service = require('child_process').spawn;
var spawn_core = require('child_process').spawn;
// *************************************************
var fp = require('find-free-port');
var chalk = require('chalk');
var fs = require('fs');
var win, serve, isDevMode;
var args = process.argv.slice(1);
serve = args.some(function (val) { return val === '--serve'; }); // prod vs dev
isDevMode = args.some(function (val) { return val === '--dev'; });
var isIntTestMode = args.some(function (val) { return val === '--int_test'; });
var isCIServer = args.some(function (val) { return val === '--ci_server'; });
if (serve) {
    require('electron-reload')(__dirname, {});
}
// Run fut-service first.
// check if not NG SERVE or whatever TODO:
var jarDir = path.join(__dirname.replace('app.asar', 'app.asar.unpacked'));
/**
 *
 * @param data
...
var core_service;
// TODO:
var SERVICE_NAME = 'fut-service-1.0.0-exec.jar';
var IO_NAME = 'fut-io-1.0.0-exec.jar';
var INT_TEST_PORT = 'int_test_port';
var INT_TEST_LOCATION = 'int_test_location';
var intTestPort;
var intTestPath;
...
var startedFutIOApplicationIn = 'Started FutIOApplication in';
var startedFutServiceApplicationIn = 'Started FutServiceApplication in';
if (isIntTestMode) {
...
var service_port = 1000;
var io_port = 2000; // TODO CHANGE LATER **************************************************************************
var servicePortCounter = 0;
// Find free port
...
function runService() {
    return new Promise(function (resolve, reject) {
        ...
        var dir = isDevMode ? 'M:/FUT Desktop App' : process.env.PORTABLE_EXECUTABLE_DIR;
        if (isIntTestMode) {
            dir = intTestPath;
        }
        ...
        runCore(dir).then(function () {
            ...
            // SERVICE **************************************
            // TODO: add this param!! ***********
            var eP = '--fut.service.endpoint=http://156.212.100.5:' + io_port + '/io';
            // TODO: *************************************
...
function runCore(dir) {
    return new Promise(function (resolve, reject) {
        var ioPath = path.join(path.join(jarDir, 'assets'), IO_NAME);
        // addToFile(ioPath, 0);
        /* '-DproxySet=true',
...
try {
    ...
    electron_1.app.on('browser-window-created', function () {
        ...
        if (isIntTestMode === false && isDevMode === false) {
            findPort(service_port).then(function (result) {
                service_port = result;
                console.log('srv: ' + service_port);
                findPort(io_port).then(function (result2) {
                    io_port = result2;
                    console.log('io: ' + io_port);
                    // Start running services
                    runService().then(function (resp) {
            ...
            }).catch(function (error) { return console.error('Error in chain: ' + error); });
        }
        else {
